...
- **Security (VLAN 4)**: 192.168.4.0/24 — UNVR Pro, AI Key, 36 cameras
- **Smart Things (VLAN 6)**: 192.168.6.0/24 — MyQ, iRobot, sprinklers
- **Guests (VLAN 7)**: 192.168.7.0/24 — Guest WiFi
- **Lighting (VLAN 20)**: 192.168.20.0/24 — 29 Hue Bridges + 20 Hue Sync Boxes
- **Gateway**: Enterprise Fortress Gateway ("VR The Mighty Fortress")
- **Fixed IP lockdown COMPLETE** (2026-02-09): 86 devices reserved via API, 21 Hue Sync boxes pre-existing. 76/86 confirmed on new IPs.
...
- **Switch SSH**: credentials in Site > Device SSH settings. `swctrl port restart id <ports> interval <secs>` bounces any port.
...
- **webOS integration NOT configured** — only Cast (Chromecast) entities exist. webOS needs physical per-TV pairing (accept prompt on TV screen). MasterCinema (.71) and DownGuest (.73) had ports open.
...
- **ATV-Bar** (.77): Apple TV 4K gen 3
- **ATV-Bar2** (.79): Apple TV 4K gen 3 (second Bar-area ATV, likely Theatre)
- **ATV-Sunroom** (.80): Apple TV 4K gen 2
- **ATV-Cabana** (.81): Apple TV 4K gen 2
- **Apple TV integration** requires physical PIN pairing (PIN shown on TV screen). Not yet paired.

## Anthem AVR Integration (2026-02-09)
- **AVR-Theatre** (.130): MRX-740, `media_player.avr_theatre`, area: Theatre, MAC `50:1e:2d:43:a0:c0` — connected
- **AVR-Master** (.131): MRX-540, `media_player.avr_master`, area: Master Cinema, MAC `50:1e:2d:43:93:72` — connected (sources: Apple TV, HDMI 2-7, eARC, Streaming, Bluetooth, Optical, Coax)
- **AVR-Sunroom** (.132): MRX SLM — **NOT COMPATIBLE** with `anthemav` integration ("cannot_receive_deviceinfo"). Known issue: [HA Community thread](https://community.home-assistant.io/t/anthemav-integration-not-working-with-new-model-receiver-mrx-slm/945647). Awaiting integration update.
...
- **SSH**: `VillaAdmin@192.168.1.2` / [password redacted] (use sshpass -f with password file, NOT inline)
- **REST API**: Token [redacted], auth via GET `/cws/api/login` → returns `authkey`; only `/rooms` endpoint useful, no device management
...
- **CPLLC**: Full site license (all 103 products). HA Platform extension installed, licensed (key: [redacted]), v4.000.0672
...
- **UniFi MCP**: `uvx --python 3.13 unifi-network-mcp`, host 192.168.1.1, port 443. Lazy-loads 67 tools via 3 meta-tools. All mutating tools require confirm=true.
...
## HDFury VRROOM Integration (2026-02-09)
...
- **Device ID**: `d03874089b0c355b4b33955c1772b171` (shared with UniFi device_tracker)
- **Area**: Theatre
- **35 entities** (hdfury + unifi platforms merged into one device):
  - `select.hdfury_vrroom_11_port_select_tx0/tx1` — HDMI input routing
  - `select.hdfury_vrroom_11_operation_mode` — operation mode
  - `switch.hdfury_vrroom_11_cec_rx0-rx3` — CEC per input
  - `switch.hdfury_vrroom_11_htpc_mode_rx0-rx3` — HTPC mode per input
  - `switch.hdfury_vrroom_11_mute_audio_tx0/tx1` — audio mute per output
  - `switch.hdfury_vrroom_11_oled_display/infrared/relay/auto_switch_inputs`
  - `sensor.hdfury_vrroom_11_output_tx0/tx1` — output status
  - `sensor.hdfury_vrroom_11_input_rx0/rx1` — input status
  - `sensor.hdfury_vrroom_11_audio_tx0/tx1/audio_output` — audio info
  - `sensor.hdfury_vrroom_11_earc_arc_status` — eARC/ARC status
  - `sensor.hdfury_vrroom_11_edid_tx0/tx1/aud/txa0/txa1/auda` — EDID info
  - `button.hdfury_vrroom_11_restart/issue_hotplug`
- **No Crestron driver available** — CPLLC has no HDFury driver in their 103-product catalog. HA is the control plane for VRROOM.
- **NVR offline** (2026-02-09): SEC-NVR at 192.168.4.7 unreachable (0 ping, ports closed). Physical issue — all 36 cameras unavailable.
